						...
						<?php
                          ...
                          if ($fpx_debitAuthCode == '00')
                            ...
                            echo $q;
                                ...
                                $ins_trans_qry="INSERT INTO transaction (merchant_id, merchant_name, user_id, user_name, user_phone, user_email, transaction_id, order_id, order_desc, status, amount, date, timestamp, source) VALUES ('1002','QYkPay','".$user_id."','".$user_name."','".$user_phone."','".$user_email."','".$order_id['transaction_id']."','".$order_id['order_id']."','".$order_id['product_description']."','successfull','".$order_id['reload_amt']."','".date('Y-m-d')."','".date('Y-m-d H:i:s')."','Website')";
                                
                                mysql_query($ins_trans_qry);
                                
                                $recharge_no=$_SESSION['number'];
                                //$url='https://taby.app/staging/fpx/epay/index.php'; // Specify your url
                                $data= array('amount'=>$order_id['reload_amt'],'retTransRef'=>$_SESSION['seller_order_no'],'productCode'=>$order_id['operator'],'transDateTime'=>$transDateTime,'date'=>$date,'msisdn'=>$recharge_no);
                                ...
                                else
                                {
                                   ...
                                   //echo $epayResponse->responseCode;
                                    ...
                                    echo $epayResponsecode = $epayResponse->responseCode;
                                    $epayProductcode = $epayResponse->productCode;
                                    $epayPinexpirydate = $epayResponse->pinExpiryDate;
                                    echo $epayPin = $epayResponse->customeFiled1;
                                    $instruction="dsdgfdsafgasfd";
