...
if(isset($_POST['confirm_password']) && isset($_POST['new_password']) && isset($_POST['old_password']))
{
    ...
    $r = mysql_query("select emp_id from admin where password = '".$oldpwd."' AND emp_id = '".$_SESSION['logid']."'");
    ...
    if($cnt == 1)
    {
        ...
        {
            $jk = mysql_query("update admin set password = '".$newpassword."' where password = '".$oldpwd."' AND emp_id = '".$_SESSION['logid']."'");
            if($jk)
                {
                    echo"success";
