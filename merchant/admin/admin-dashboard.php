...
<div class="hbox hbox-auto-xs hbox-auto-sm" ng-init="
  ...
  <div class="col">
    ...
    <div class="wrapper-md" ng-controller="FlotChartDemoCtrl">
      ...
      <div class="row">
<?php
$billing_query = "select * from balance";
...
?>      
      ...
      <div class="panel panel-default">
        ...
        <footer class="panel-footer bg-info text-center no-padder">
          <div class="row no-gutter">
            <div class="col-xs-4">
              <div class="wrapper">
                <span class="m-b-xs h3 block text-white">$<?php echo $balance_data['balance']; ?></span>
                <small class="text-muted">Main Balance</small>
              </div>
            </div>
            <div class="col-xs-4 dk">
              <div class="wrapper">
                <span class="m-b-xs h3 block text-white">$<?php echo $balance_data['reserved_balance']; ?></span>
                <small class="text-muted">Reserved Balance</small>
              </div>
            </div>
            <div class="col-xs-4">
              <div class="wrapper">
                <span class="m-b-xs h3 block text-white">$<?php echo $balance_data['balance']+$balance_data['reserved_balance']; ?></span>
    ...
    <?php
            ...
            $query = mysql_query("select cheque_status from cheque where cheque_status='Pending'");
            $pending_row = mysql_num_rows($query);
            $query1 = mysql_query("select cheque_status from cheque where cheque_status='Declined'");
            $declined_row = mysql_num_rows($query1);
            $query2 = mysql_query("select cheque_status from cheque where cheque_status='Processed'");
            $processed_row = mysql_num_rows($query2);
            $ticket_open = mysql_query("select ticket_type from support where ticket_type='Open'");
            $ticket_open_cnt = mysql_num_rows($ticket_open);
            $ticket_closed = mysql_query("select ticket_type from support where ticket_type='Closed'");
            $ticket_closed_cnt = mysql_num_rows($ticket_closed);
            $users = mysql_query("select user_id from users");
            $user_cnt = mysql_num_rows($users);
            $amount_query = mysql_query("select amount from cheque where cheque_status = 'Pending'");
            $pending_amt = 0;
            while($pending_amount = mysql_fetch_array($amount_query))
            {
                
               $pending_amt = $pending_amount['amount']+$pending_amt;
            }
            $declined_query = mysql_query("select amount from cheque where cheque_status = 'Declined'");
            $declined_cnt = mysql_num_rows($declined_query);
            $declined_amt = 0;
            while($declined_amount = mysql_fetch_array($declined_query))
            {
                
               $declined_amt = $declined_amount['amount']+$declined_amt;
            }
            $processed_query = mysql_query("select amount from cheque where cheque_status = 'Processed'");
            $processed_amt = 0;
            while($processed_amount = mysql_fetch_array($processed_query))
            {
                
               $processed_amt = $processed_amount['amount']+$processed_amt;
            }
        ...
        <div class="col-md-6">
          <div class="row row-sm text-center">
            <div class="col-xs-6">
            <a href="pending-amounts.php" target="_blank">
              <div class="panel padder-v item">
                <div class="h1 text-info font-thin h1"><?php echo $pending_amt;?></div>
                <span class="text-muted text-xs">Pending Amounts</span>
            ...
            <div class="col-xs-6">
              <a href="declined-amounts.php" class="block panel padder-v bg-primary item" target="_blank">
                <span class="text-white font-thin h1 block"><?php echo $declined_amt;?></span>
                <span class="text-muted text-xs">Declined Amounts</span>
            ...
            <div class="col-xs-6">
              <a href="processed-amounts.php" class="block panel padder-v bg-info item" target="_blank">
                <span class="text-white font-thin h1 block"><?php echo $processed_amt;?></span>
                <span class="text-muted text-xs">Processed Amounts</span>
                <span class="top">
            ...
            <div class="col-xs-6">
            <a href="open-ticket.php" target="_blank">
              <div class="panel padder-v item">
                <div class="font-thin h1"><?php echo $ticket_open_cnt;?></div>
                <span class="text-muted text-xs">Open Ticket</span>
                <div class="bottom">
                  <i class="fa fa-caret-up text-warning m-l-sm m-r-sm"></i>
                </div>
              </div>
            ...
            <div class="col-xs-12 m-b-md">
              <div class="r bg-light dker item hbox no-border">
                ...
                <div class="col dk padder-v r-r">
                  <div class="text-primary-dk font-thin h1"><span><?php echo $total_payout;?></span></div>
                  <span class="text-muted text-xs">Total Payout</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="col-sm-12">
          <div class="panel panel-default">
            ...
            <div class="text-center wrapper m-b-sm">              
              ...
              [<?php echo $cheque_data['pending_cheque']; ?>, <?php echo $cheque_data['processed_cheque']; ?>, <?php echo $cheque_data['declined_cheque']; ?>], 
              {
                type:'pie', 
                height:126, 
            ...
            <ul class="list-group no-radius">
              ...
              <a href="pending-cheques.php" target="_blank">
                <span class="pull-right"><?php echo $cheque_data['pending_cheque']; ?></span>
                <span class="label bg-primary">1</span>
                Pending Cheque
              </a>
              </li>
              <li class="list-group-item">
              <a href="processed-cheques.php" target="_blank">
                <span class="pull-right"><?php echo $cheque_data['processed_cheque']; ?></span>
                <span class="label bg-info">2</span>
                Processed Cheque
                </a>
              </li>
              <li class="list-group-item">
              <a href="returned-cheques.php" target="_blank">
                <span class="pull-right"><?php echo $cheque_data['declined_cheque']; ?></span>
...
$chart_query = "
SELECT user_id,
           SUM(CASE WHEN MONTH(date) = '".$current_month."' AND cheque_status = 'Processed' THEN amount ELSE 0 END) AS 'current_month',
           SUM(CASE WHEN MONTH(date) = '".$last_month."' AND cheque_status = 'Processed' THEN amount ELSE 0 END) AS 'last_month',
           SUM(CASE WHEN MONTH(date) = '".$last_second_month."' AND cheque_status = 'Processed' THEN amount ELSE 0 END) AS 'last_second_month',
           SUM(CASE WHEN MONTH(date) = '".$last_third_month."' AND cheque_status = 'Processed' THEN amount ELSE 0 END) AS 'last_third_month',
           SUM(CASE WHEN MONTH(date) = '".$last_fourth_month."' AND cheque_status = 'Processed' THEN amount ELSE 0 END) AS 'last_fourth_month',
           SUM(CASE WHEN MONTH(date) = '".$last_fifth_month."' AND cheque_status = 'Processed' THEN amount ELSE 0 END) AS 'last_fifth_month'
    FROM   cheque
    ORDER BY cheque_id DESC
";
...
$chart_data = mysql_fetch_array($chart_sql); 
 ...
 ?>
       ...
       <div class="col-md-6">
        <div class="panel panel-default">
          <div class="panel-heading font-bold">Processed Amount</div>
          <div class="panel-body">
        ...
        <div class="wrapper-md">
          <!--<div class="m-b-sm text-md">Who to follow</div>-->
              ...
              <div class="padder-md">      
                  ...
                  <?php 
                    $comments1 = "select * from comment ORDER BY comment_id DESC limit 5";
                    ...
                    <div class="sl-item">
                      <div class="m-l">
                        ...
                        <p><a href class="text-info"><?php echo $user_row['f_name']."".$user_row['l_name'];?></a><br /><a href="view-cheque-detail.php?id=<?php echo $comments_rows['cheque_id'];?>" class="text-info" target="_blank">Cheque #<?php echo $comments_rows['cheque_id'];?></a><br />  <?php echo $comments_rows['comment'];?></p>
                      </div>
                    </div>
        ...
        <div class="wrapper-md">
          <!--<div class="m-b-sm text-md">Chat</div>-->
            ...
            <div class="padder-md">      
                  ...
                  <?php 
                    $comments = "select * from support_comment order by support_comment_id DESC limit 5";
...
<script src="../theme/html/js/admin-ui-jp.config.js"></script>
