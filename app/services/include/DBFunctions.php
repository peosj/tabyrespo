...
class DBFunctions{
    private $db;
    public $base_url = "http://www.qykpay.com/app/services/";
    ...
    public function getReload($reload_amount, $merchant_name, $merchant_id, $first_name, $last_name, $email,$phone, $user_id,$prod_desc)
    {
        $order_id = date('QYK-YmdHis');
        $date = date("Y-m-d");
        ...
        $query = "insert into orders(reload_amt, total, merchant_id, merchant_name, product_description, user_id, order_id, date, time_stamp, seller_order_no, source, transaction_type, payment_status) values('".$reload_amount."', '".$reload_amount."', '".$merchant_id."', '".$merchant_name."', '".$prod_desc."', '".$user_id."', '".$order_id."', '".$date."', '".$timestamp."', '".$order_id."',  'App',  'Merchant Payment',  '0')";
    ...
    public function getCurrentCycle($user_id)
    {
        ...
        $query = "select * from orders where user_id = '".$user_id."' AND date BETWEEN '".$date."' AND '".$till_15."' ORDER BY sno DESC";
                ...
                $txn .= '
						...
						<div class="row" style="margin-top: 0px;">
                          <div class="col" style="text-align: center;width: 100%;">                           
                            <div style="width: 180px;font-size: 14px;color: #8b8a8a;font-weight: bold; margin: 0 auto;">
                                ...
                                Amount Spent
                                <br />
                                <span style="color: #222222;">RM '.$row['total'].'</span>
                                </div>                                 
                                 <ring class="c50"></ring>                                 
                            </div>
                          </div>                          
                        ...
                        <div class="row">
                          ...
                          <div class="col col-50" style="text-align: right;font-size:12px; font-weight: bold;color:#b4b2b2;"> 
                            AVAILABLE CREDIT<br />
                            <span style="color: #555555;">RM '.$row['total'].'</span>                            	
                          </div>                          
                        </div>
    ...
    public function getTransactionHistory($user_id)
    {
        ...
        $query = "select COALESCE(sum(total),0) from orders where user_id = '".$userid."' AND (date between '".$lower_date."' and '".$upper_date."') AND transaction_type != 'Batch' ";
        $data = mysql_fetch_array(mysql_query($query));
        array_push($amount_array,$data['COALESCE(sum(total),0)']);
        array_push($range_array,$lower_date.'$$'.$upper_date);
        array_push($userid_array,$userid);
        
        $query_due = "select COALESCE(sum(total),0) from orders where user_id = '".$userid."' AND (date between '".$lower_date."' and '".$upper_date."') AND payment_status != '1' AND transaction_type != 'Batch'";
        $due_amount = mysql_fetch_array(mysql_query($query_due));
        array_push($due_amount_array,$due_amount['COALESCE(sum(total),0)']);
				...
				<div class="list" style="background: #ffffff;margin-top: 6px;margin: 10px;margin-top: 10px;">
                    <div class="item" style="padding-bottom: 0px;">
                        ...
                        <div class="row" style=" margin-top: 15px;">                                          
                          ...
                          <div class="col col-33" style="text-align: center;font-size:12px; font-weight: bold;color:#b4b2b2; border-right:1px solid #f1f1f1;">
                            Amount Spent<br />
                          ...
                          <div class="col col-33" style="text-align: center;font-size:12px; font-weight: bold;color:#b4b2b2; border-right:1px solid #f1f1f1;">
                            Due Amount<br />
                            <span style="color: #555555;">RM '.$due_amount_array[$i].'</span>                                                        	
                          </div>                
                          
                          <div class="col col-33" style="text-align: right;font-size:12px; font-weight: bold;color:#b4b2b2; margin-bottom: 15px;">';
                          
                          if($due_amount_array[$i] == 0)
                          {
                            $txn .= '
                                <button ng-click="" class="button button-balanced" style="width: 100%;min-height: 34px; line-height: 34px;background-color: #737373;">
    								<span class="font-family-default" style="font-size: 12px; font-weight: 600 !important;">PAID</span>
    							</button>';
                          }
                          else
                          {
                            $txn .= '
                                <button ng-click="open_google(\''.$due_amount_array[$i].'\',\''.$from_to_till_date.'\',\''.$from_to_till_dates.'\');" class="button button-balanced" style="width: 100%;min-height: 34px; line-height: 34px;">
    								<span class="font-family-default" style="font-size: 12px; font-weight: 600 !important;">PAY NOW</span>
    							</button>';
                          }
    ...
    public function  showTransactionHistory($user_id,$from_till_date,$from_till_dates){
        ...
        //var_dump($date);exit;
        $query = "select * from orders where user_id = '".$user_id."' AND (date between '".$dates[0]."' AND '".$dates[1]."') AND transaction_type != 'Batch' ORDER BY transaction_id DESC";
        ...
        if($no_of_rows>0){
            ...
            $temp_data1='<div class="list" style="margin: 10px;margin-top: 15px; background: #48a79c;">
				...
				<div class="list" style="background: #ffffff;margin-top: 6px;margin: 10px;margin-top: 10px;">
                    <div class="item" style="padding-bottom: 0px;">
                        ...
                        <div class="row" style=" margin-top: 15px;">
                          ...
                          <div class="col col-33" style="text-align: right;font-size:12px; font-weight: bold;color:#b4b2b2; margin-bottom: 15px;">'; 
                                $temp_data1_2_Pay_btn = '<button ng-click="open_google();" class="button button-balanced" style="width: 100%;min-height: 34px; line-height: 34px;">
                            								<span class="font-family-default" style="font-size: 12px; font-weight: 600 !important;">PAY NOW</span>
                            							</button>';
                                $temp_data1_2_Paid_btn = '<button ng-click="" class="button button-balanced" style="width: 100%;min-height: 34px; line-height: 34px;background-color: #737373;">
                            								<span class="font-family-default" style="font-size: 12px; font-weight: 600 !important;">PAID</span>
                            							</button>';                      	
                          $temp_data1_3='</div>                          
                        </div>
             ...
             while($result = mysql_fetch_array($result1))
             {
                ...
                {
                    $merchant_operator = $result['operator'];
                }
                else
                {
                    $merchant_operator = $result['merchant_name'];
                }
                
                
                    $temp_amount = $temp_amount + $result['total'];
                
                    $payment_status = "Paid";
                    $payment_color = "#3cbf3f";
                    
                    if($result['payment_status'] != '1')
                    {
                        $temp_due_amount = $temp_due_amount + $result['total'];
                        $payment_status = "Pending";
                        $payment_color = "#ff8830";
                    }
                  ...
                  $temp_data2.='                      
                    <div class="item" style="border: none; padding: 10px;">
                        ...
                        <div class="row" style="font-size: 10px;border-bottom:1px solid #dbdbdb; ">
                            <div class="col col-80" style="overflow:hidden">
                                '.$result['product_description'].'
                            </div>
                            <div class="col" style="text-align: right;">
                                <span style="color: '.$payment_color.';">'.$payment_status.'</span>
                            </div>
                        </div>
                    </div>';
        ...
        else
        {
            $temp_data4='<div class="list card customcard">
                    <div class="row col col-100" style="text-align: center;text-transform: uppercase;font-weight: bold;">                           
        ...
        if($temp_due_amount == 0)
        {
            $btn = $temp_data1_2_Paid_btn;
        }
        else
        {
            $btn = $temp_data1_2_Pay_btn;
        }
        
        $data = $temp_data1.$temp_amount.$temp_data1_1.$temp_due_amount.$temp_data1_2.$btn.$temp_data1_3.$temp_data2.$temp_data3.$temp_data4.'qykpay11'.$dates[0].'qykpay11'.$dates[1].'qykpay11'.$temp_amount.'qykpay11'.$temp_due_amount;
    ...
    public function getCurrentBillCycle($user_id)
    {
        $qq = mysql_query("select * from users where phone = '".$user_id."'");
        $qqq = mysql_fetch_array($qq);
        $userid = $qqq['user_id'];
        ...
        $txn = "";
        $today = date('Y-m-d');
        $todays_day = date("d",strtotime($today));
        $todays_month = date("m",strtotime($today));
        $todays_year = date("Y",strtotime($today));
        
        if($todays_day<=15)
        {
            $lower_date = date('Y-m-01');
            $upper_date = $today;
        }
        else
        {
            $lower_date = date('Y-m-16');
            $upper_date = date('Y-m-d',strtotime("last day of this month"));
        }
        
        
        $query = "select COALESCE(sum(total),0) from orders where user_id = '".$userid."' AND (date between '".$lower_date."' and '".$upper_date."') AND transaction_type != 'Batch' ";
        $data = mysql_fetch_array(mysql_query($query));
        $amount = $data['COALESCE(sum(total),0)'];
        
        $query_due = "select COALESCE(sum(total),0) from orders where user_id = '".$userid."' AND (date between '".$lower_date."' and '".$upper_date."') AND payment_status != '1' AND transaction_type != 'Batch'";
        $due_amount = mysql_fetch_array(mysql_query($query_due));
        $amount_due = $due_amount['COALESCE(sum(total),0)'];
        
        $from_to_till_date = $lower_date.' to '.$upper_date;
        $from_to_till_dates = $lower_date.'||'.$upper_date;
				...
				<div class="list" style="background: #ffffff;margin-top: 6px;margin: 10px;margin-top: 10px;">
                    <div class="item" style="padding-bottom: 0px;">
                        ...
                        <div class="row" style=" margin-top: 15px;">                                          
                          ...
                          <div class="col col-33" style="text-align: center;font-size:12px; font-weight: bold;color:#b4b2b2; border-right:1px solid #f1f1f1;">
                            Amount Spent<br />
                            <span style="color: #555555;">RM '.$amount.'</span>                                                         	
                          </div>  
                          
                          
                          <div class="col col-33" style="text-align: center;font-size:12px; font-weight: bold;color:#b4b2b2; border-right:1px solid #f1f1f1;">
                            Due Amount<br />
                            <span style="color: #555555;">RM '.$amount_due.'</span>                                                        	
                          </div>                
                          ...
                          {
                            $txn .= '
                                <button ng-click="open_google(\''.$amount_due.'\',\''.$lower_date.'\',\''.$upper_date.'\');" class="button button-balanced" style="width: 100%;min-height: 34px; line-height: 34px;">
    								<span class="font-family-default" style="font-size: 12px; font-weight: 600 !important;">PAY NOW</span>
    							</button>';
                          }
    ...
    public function getMobileOperators()
    {
        ...
        while($row = mysql_fetch_array($query))
        {
           ...
           $operator .= "<option value='".$row['product_id']."qykpay11".$row['name']."'>".$row['name']."</option>";
        } 
        
        return $operator;
    }
    
    public function getMobileReloads($operator)
    {
        $reloads = "";
        $need3 = mysql_query("SELECT * FROM `product_list` WHERE `product_id`='".$operator."' ");        
        while($need_row = mysql_fetch_array($need3))
        
                {        
                    $label11l = $need_row['title'];         
                    $sub_cat_id = number_format($need_row['denomination'],"2",".","");
                    
                    $reload_amount= $need_row['reload_amount']/100;
                    $gst= $need_row['gst']/100;
                                                           
                    $reloads .="<option value='".$sub_cat_id."qykpay11".$reload_amount."qykpay11".$gst."'>$label11l</option>";        
                }
        return $reloads;
    }
